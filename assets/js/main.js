...
jQuery( document ).ready( function( $ ) {
	...
	if ( currentPage.postType && ( currentPage.postType === 'videos' || currentPage.postType === 'live-streams' || pageURL.includes( 'free-bass-lessons' ) ) ) {
		commentVideoEmbed();
	}
	...
	function commentVideoEmbed() {
		...
		if ( $( '.comment-content > p' ).length ) {
			const commentContent = document.querySelectorAll( '.comment-content > p' );
	...
	$( '.play_video' ).on( 'click', function() {
		...
		commentsAjaxCall( ajaxURL, postID )
	        ...
	        let html = '<div class="full_width lesson_title">' +
		        '<h3>' + videoTitle + '</h3>' +
		        '</div>' +
                videoDesc +
