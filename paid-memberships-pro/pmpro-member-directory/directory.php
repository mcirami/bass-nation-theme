	...
	<div class="container">
		<div class="directory_wrapper">
						...
						<div id="pmpro_member-<?php echo esc_attr( $auser->ID ); ?>" class="<?php echo pmpro_get_element_class( 'pmpro_member_directory-item', 'directory_item' ); ?>">
							<?php if(!empty($show_avatar)) { ?>
								<div class="pmpro_member_directory_avatar">
									<?php if(!empty($link) && !empty($profile_url)) { ?>
										<a target="_self" class="<?php echo $avatar_align; ?>" href="<?php echo esc_url( pmpromd_build_profile_url( $auser ), $profile_url ); ?>"><?php echo get_avatar($auser->ID, $avatar_size, NULL, $auser->display_name); ?></a>
									<?php } else { ?>
							...
							<div class="content_wrap">
								<div class="pmpro_member_directory_display-name span">
									<?php if(!empty($link) && !empty($profile_url)) { ?>
										<div class="label">
								...
								<?php } ?>
								<?php if(!empty($show_level)) { ?>
									<div class="pmpro_member_directory_level span">
										<div class="label">
											<p>
									...
									<div class="span">
										<div class="label">
											<p class="pmpro_member_directory_date">
												<strong><?php _e('Join Date', 'pmpro-member-directory'); ?></strong>
											</p>
										</div>
										<p><?php echo date_i18n(get_option("date_format"), $auser->membership_level->startdate); ?></p>
									</div>
								<?php } ?>
								<?php
								if(!empty($user_fields_array))
			...
			<div class="pmpro_pagination">
				...
				$counter = 0;
				if ( empty( $pn ) || $pn != 1 ) {
					echo '<a target="_self" href="' . esc_url( add_query_arg( $query_args, get_permalink( $post->ID ) ) ) . '" title="' . esc_attr__( 'Previous', 'pmpromd' ) . '">...</a>';
				}
				...
				if( round( $number_of_pages, 0 ) !== 1 && $pn !== 1 ) {
					...
					for( $i = $pn; $i <= $number_of_pages; $i++ ){
						if( $counter <= 6 ){
							...
							echo '<a target="_self" href="' . esc_url( add_query_arg( $query_args, get_permalink( $post->ID ) ) ) . '" ' . $active_class . ' title="' . esc_attr( sprintf( __('Page %s', 'pmpromd' ), $i ) ) . '">' . $i . '</a>';
						}
						$counter++;
					}
				}
