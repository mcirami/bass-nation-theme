		...
		if(!empty($pu))
		{
			...
			<div id="pmpro_member_profile-<?php echo esc_attr( $pu->ID ); ?>" class="light_gray_bg box_shadow <?php echo pmpro_get_element_class( 'pmpro_member_profile', 'profile'); ?>">
				...
				<div class="content_wrap">
					...
					<?php if(!empty($show_startdate)) { ?>
						<div class="member_info">
							<div class="label pmpro_member_directory_date">
					...
					<?php if(!empty($show_level)) { ?>
						<div class="member_info membership_level">
							<div class="label pmpro_member_directory_level">
					...
					<?php } ?>
						...
						if(!empty($fields_array))
						{
							foreach($fields_array as $field)
							{
								...
								{
									?>
									<div class='member_info <?php echo esc_attr($field[1]); ?>'>
										<div class="pmpro_member_directory_<?php echo esc_attr($field[1]); ?>">
											...
											if(is_array($meta_field) && !empty($meta_field['filename']) ) {
												...
												<p><?php echo pmpromd_display_file_field($meta_field); ?></p>
												<?php
											...
											else
											{
												...
												<label>
													...
													<p>
													<?php
													echo pmpromd_format_profile_field( $meta_field, $field[1] ) . "</p>";
												}
											}
											?>
						...
						if(!empty($show_bio) && !empty($pu->description) ) { ?>
							<div class="member_info directory_bio">
								<div class="label pmpro_member_directory_bio">
									<strong><?php _e('Bio', 'pmpro-member-directory'); ?></strong>
								</div>
			...
			<p class="pmpro_actions_nav">
				...
				if ( ! empty( $directory_url ) ) {
					$pmpro_member_profile_action_links['view-directory'] = sprintf( '<a target="_self" class="button black" id="pmpro_actionlink-view-all-members" href="%s">%s</a>', esc_url( $directory_url ), esc_html__( 'View All Members', 'pmpro-member-directory' ) );
				}
				...
				if ( ! empty( $pu ) && $pu->ID === $current_user->ID ) {
					...
					if ( ! empty( $edit_profile_url) ) {
						$pmpro_member_profile_action_links['edit-profile'] = sprintf( '<a target="_self" class="button red" id="pmpro_actionlink-profile" href="%s">%s</a>', esc_url( $edit_profile_url ), esc_html__( 'Edit Profile', 'paid-memberships-pro' ) );
					}
				}
