...
1. Alternatively install it using dedicated widget in `Appearance -> Widgets`, it's called `Wise Chat DirectWindow`. See below for details.
...
You can install Wise Chat using dedicated widget. Go to `Appearance -> Widgets` and add `Wise Chat DirectWindow` widget to a sidebar. In the widget's settings you can specify:
...
or put "Wise Chat DirectWindow" widget on the desired sidebar and set the desired channel name in Channel field.
...
Go to Appearance -> Widgets page, drag and drop "Wise Chat DirectWindow" widget on the desired sidebar. The name of the channel can be specified as well.
...
= 3.6.3 =
* Bans: option to ban by user (aside from IP address)
* Chat button - a new version
* Fixed: warnings in Elementor integration
* Fixed: deleting channel error

= 3.6.2 =
* Muting users: an icon that indicated a muted user
* Fixed: empty window after a fresh installation with [wise-chat] shortcode

= 3.6.1 =
* Muting users: option to mute by user ID (default mode)
* Option to enable / disable images, attachments or voice messages in an own-created channel
* Fixed: broken style of the submit button on mobile devices
* Fixed: message reverted to previous after edit

= 3.6 =
* Add, edit and browse channels

= 3.5.7 =
* Fixed: resetting YT videos
* Fixed: users counter invisible with disabled browser

= 3.5.6 =
* Fixed: blank chat after message approval in a pending message
* Fixed: issue with BuddyPress group chat
...
= 3.4 =
* Live Chat edition: Wise Chat Live Chat widget
* Live Chat edition: [wise-chat-live-chat] shortcode for enabling live chat mode
* Live Chat edition: Live chat operator page
...
= 3.2.2 =
* Fixed: Missing BP group chat with disabled public channels
...
= 3.0.3 =
* Fixed: "This name is already occupied" warning with disabled username duplication check
...
= 3.0 =
* Plugin re-implemented from the scratch
* Completely new mobile version of the chat
