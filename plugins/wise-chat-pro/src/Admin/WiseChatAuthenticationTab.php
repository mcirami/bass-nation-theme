...
class WiseChatAuthenticationTab extends WiseChatAbstractTab {
	...
	private static function getAuthModes() {
		return array(
			...
			'external' => array('External', 'Uses external services to authenticate in Wise Chat Pro<br/><strong>Notice: </strong> PHP 8.0 is required to enable the external authentication. Current version of PHP: '.PHP_VERSION),
		);
	}
	...
	public function customFieldsCallback() {
		...
		for ($i = 1; $i <= 7; $i++) {
			...
			$nameInput = sprintf(
				...
				WiseChatOptions::OPTIONS_NAME, $key, htmlspecialchars(isset($custom[$key]) ? $custom[$key]['name'] : '')
			);
			$typeSelect = sprintf(
