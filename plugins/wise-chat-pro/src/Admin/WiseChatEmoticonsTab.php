...
use Kainex\WiseChatPro\Model\WiseChatEmoticon;
...
class WiseChatEmoticonsTab extends WiseChatAbstractTab {
	...
	public function emoticonsCallback() {
		...
		foreach ($emoticons as $key => $emoticon) {
			...
			$actions = array();
			$deleteURL = admin_url("options-general.php?page=".WiseChatSettings::MENU_SLUG."&wc_action=deleteCustomEmoticon&id=".urlencode($emoticon->getId())).'&wc_tab=emoticons';
			$moveUpURL = admin_url("options-general.php?page=".WiseChatSettings::MENU_SLUG."&wc_action=moveUpCustomEmoticon&id=".urlencode($emoticon->getId())).'&wc_tab=emoticons';
			$moveDownURL = admin_url("options-general.php?page=".WiseChatSettings::MENU_SLUG."&wc_action=moveDownCustomEmoticon&id=".urlencode($emoticon->getId())).'&wc_tab=emoticons';
			if ($key > 0) {
				$actions[] = sprintf(
