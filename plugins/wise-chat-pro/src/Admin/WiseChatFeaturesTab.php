...
class WiseChatFeaturesTab extends WiseChatAbstractTab {
	...
	public function customReactionsCallback() {
		...
		for ($i = 1; $i <= 7; $i++) {
			...
			$actionInput = sprintf(
				'<input type="text" name="%s[reactions_custom][%d][action]" value="%s" maxlength="100" style="max-width: 100px;">%s',
				WiseChatOptions::OPTIONS_NAME, $key, htmlspecialchars($custom[$key]['action']), $i === 1 ? '<p class="description">e.g. Like</p>' : ''
			);
			$activeInput = sprintf(
				'<input type="text" name="%s[reactions_custom][%d][active]" value="%s" maxlength="100" style="max-width: 100px;">%s',
			...
			$imageInput = sprintf(
				...
				'<button class="wc-image-picker button-secondary" data-target-id="reactions_custom_%d_image" data-image-container-id="reactions_custom_%d_image_container">Browse</button>%s',
			...
			$imageSmInput = sprintf(
				...
				'<button class="wc-image-picker button-secondary" data-target-id="reactions_custom_%d_image_sm" data-image-container-id="reactions_custom_%d_image_sm_container">Browse</button>%s',
