...
class WiseChatModerationTab extends WiseChatAbstractTab {
	...
	public function moderatorAddCallback() {
		$url = admin_url("options-general.php?page=".WiseChatSettings::MENU_SLUG."&wc_action=addModerator&nonce=".wp_create_nonce('addModerator'));

		$checkboxes = array();
		foreach ($this->rights as $slug => $name) {
			$checkboxes[] = sprintf('<label><input type="checkbox" value="%s" id="addModerator-%s" name="addModerator-%s" class="wc-add-moderator-right" />%s</label>', $slug, $slug, $slug, $name);
		}

		printf(
			'<input type="text" value="" placeholder="User Login" class="wcUserLoginHint wc-add-moderator-user-login" style="margin-bottom: 10px;" /><br />%s<br />'.
			'<a class="button-secondary wc-add-moderator-button" href="%s" title="Adds user to the moderators list" style="margin-top: 10px;">Add</a>',
