	...
	public function hasCurrentBpUserRight($rightName) {
		...
		if ($wpUser !== null && $groupId > 0) {
			if ($this->options->isOptionEnabled('enable_buddypress', false) && function_exists("is_buddypress")) {
				...
				if ($rightName == 'mute_user') {
					if (groups_is_user_mod($wpUser->ID, $groupId) && groups_get_groupmeta($groupId, 'bp_wisechat_permissions_mod_mute_users')) {
						return true;
					}
					if (groups_is_user_admin($wpUser->ID, $groupId) && groups_get_groupmeta($groupId, 'bp_wisechat_permissions_admin_mute_users')) {
						return true;
					}
				}
			}
