...
class WiseChatMessagesDAO {
	...
	*/
	 ...
	 */
	public function __construct(WiseChatChannelsDAO $channelsDAO, WiseChatUsersDAO $usersDAO) {
		...
		$sql = sprintf("SELECT o.*
			FROM `%s` o
		  	LEFT JOIN `%s` b ON o.chat_recipient_id = b.chat_recipient_id 
			      	AND o.time < b.time AND b.channel_id = '%d' AND b.chat_recipient_id IS NOT NULL
			      	AND b.chat_recipient_id != '' AND b.chat_user_id = %d AND b.admin = 0
			WHERE b.time IS NULL AND o.channel_id = '%d' AND o.chat_recipient_id IS NOT NULL
			      	AND o.chat_recipient_id != '' AND o.chat_user_id = %d AND o.admin = 0
			ORDER BY o.time DESC LIMIT %d
			", $this->table, $this->table, $channel->getId(), $user->getId(), $channel->getId(), $user->getId(), $limit);
	...
	public static function populateData($messageRawData) {
		...
		$message->setChannelId($messageRawData->channel_id);
	...
	*/
	public function getChannelsSummary() {
		...
		$channelsTable = WiseChatInstaller::getChannelsTable();
