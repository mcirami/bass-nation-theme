...
class WiseChatUITemplates {
	 ...
	 */
	public function __construct(WiseChatChannelUsersDAO $channelUsersDAO, WiseChatUserService $userService, WiseChatRenderer $renderer, WiseChatUsersDAO $usersDAO, WiseChatClientSide $clientSide, WiseChatOptions $options) {
	...
	public function getInfoWindow($user) {
		if ($user->getWordPressId() > 0) {
			...
			$templateDefault = "{role}";
			$template = $this->options->getOption('users_list_info_window_template_ext_auth', $templateDefault);
		} else {
	...
	private function getAnonymousUserVariables($user) {
		...
		$variables['role'] = $variables['roles'] = __('Anonymous user', 'wise-chat-pro');
	...
	private function getExternalUserVariables($user) {
		...
		switch ($user->getExternalType()) {
			case 'fb':
				$variables['role'] = $variables['roles'] = __('Facebook user', 'wise-chat-pro');
				break;
			case 'tw':
				$variables['role'] = $variables['roles'] = __('Twitter user', 'wise-chat-pro');
				break;
			case 'go':
				$variables['role'] = $variables['roles'] = __('Google user', 'wise-chat-pro');
				break;
			default:
				$variables['role'] = $variables['roles'] = __('Anonymous user', 'wise-chat-pro');
		}
