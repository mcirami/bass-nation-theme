...
namespace Kainex\WiseChatPro\Services\Channels\Listing;
...
class WiseChatChannelsSourcesService {
	 ...
	 */
	public function __construct(WiseChatAuthentication $authentication, WiseChatUserService $userService, WiseChatService $service, WiseChatChannelsService $channelsService, WiseChatChannelUsersDAO $channelUsersDAO, WiseChatUsersDAO $usersDAO, WiseChatOptions $options) {
	...
	public function getChannels() {
		return array_merge($this->channelsService->getConstantChannels(), $this->channelsService->getUserChannels($this->authentication->getUserIdOrNull()));
	}
	...
	public function getPublicChannelByName($name) {
		return $this->channelsService->getByName($name);
	}
	...
	private function isChannelUserVisible($channelUser) {
		...
		// hide users
		$excludeUsers = (array) $this->options->getOption('users_list_hide_users', array());
		if (in_array($channelUser->getUser()->getWordPressId(), $excludeUsers)) {
			return false;
		}
	 ...
	 *
	 * @param ChannelUser[] $channelUsers
	 * @return ChannelUser[]
	 */
	private function appendOfflineUsers($channelUsers) {
		...
		$accessUsers = $this->options->getOption('access_users', array());
		$excludeUsers = (array) $this->options->getOption('users_list_hide_users', array());
		$wpUsers = $this->usersDAO->getWPUsers(array('include' => is_array($accessUsers) ? $accessUsers : array(), 'exclude' => $excludeUsers));
