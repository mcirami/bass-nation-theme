...
use Kainex\WiseChatPro\Services\WiseChatGeoService;
...
class WiseChatAuthentication {
	 ...
	 * @param WiseChatGeoService $geoService
	 */
	public function __construct(WiseChatUsersDAO $usersDAO, WiseChatChannelUsersDAO $channelUsersDAO, WiseChatOptions $options, WiseChatGeoService $geoService) {
    ...
    public function isAuthenticatedExternally() {
        ...
        return $user !== null && $user->getExternalType();
    }
    ...
    public function validateUserName($userName) {
        ...
        if (!$userName || !preg_match('/^[\p{L}a-zA-Z0-9\-_ ’]+$/u', $userName)) {
            throw new Exception(__('Only letters, number, spaces, hyphens and underscores are allowed', 'wise-chat-pro'));
        }
        ...
        $occupiedException = new Exception(__('This name is already occupied', 'wise-chat-pro'));
        $prefix = __('Anonymous', 'wise-chat-pro');
