 ...
 */
class WiseChatUserService extends WiseChatUsersDAO {
	...
	*/
	 ...
	 */
	public function __construct(WiseChatClientSide $clientSide, WiseChatActions $actions, WiseChatMessagesDAO $messagesDAO, WiseChatChannelUsersDAO $channelUsersDAO, WiseChatAuthentication $authentication, WiseChatAuthorization $authorization, WiseChatUserEvents $userEvents, WiseChatOptions $options) {
	...
	public function autoAuthenticateOnMaintenance() {
		...
		if ($user !== null) {
			$this->actions->publishAction(
				'refreshPlainUserName', array('name' => $user->getName()), $user->getId()
			);
	...
	*/
	...
	public function onWpUserProfileUpdate($wpUserId, $oldWpUser) {
		...
		if ($wpUser !== null && $oldWpUser !== false) {
			...
			$this->updateNameByWordPressId($userName, $wpUser->ID);
