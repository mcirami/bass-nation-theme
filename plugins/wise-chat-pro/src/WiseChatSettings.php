...
use Kainex\WiseChatPro\Admin\WiseChatAbstractTab;
...
class WiseChatSettings {
	...
	const MENU_TITLE = WISE_CHAT_PRO_NAME.' Settings';
	
	const SECTION_FIELD_KEY = '_section';
	const CAPABILITY = 'manage_'.WISE_CHAT_PRO_SLUG.'_options';
	...
	*/
	...
	public function __construct() {
		if (!defined('WISE_CHAT_PRO_VERSION_LIVE')) {
			unset($this->tabs['wise-chat-liveChat']);
		}
	}
	...
	*/
	...
	public function enqueueScripts() {
		...
		wp_enqueue_style('wise_chat_pro_admin',  plugins_url('../assets/css/wise-chat-pro-admin.css?v='.WISE_CHAT_PRO_VERSION, __FILE__));
	}
	...
	*/
	private function getTabObject($tabKey) {
		...
		$classPathAndName = "\Kainex\WiseChatPro\Admin\WiseChat{$tabKey}Tab";

		/** @var WiseChatAbstractTab $tab */
	...
	private function renderEndOfSupport() {
		if (WiseChatServiceConnector::getSettings(1)) {
			return;
		}
		?>
			<div class='postbox' style="border-left: 4px solid #ff2424; border-right: 4px solid #ff2424; border-radius: 5px;">
				<h3 class='hndle'><span>Notice: End of support</span></h3>
				<div class='inside'>
					<p class='description'>
						<span style='display: inline-block; font-size: 1.2em'>
							<?php echo WISE_CHAT_PRO_NAME; ?> Plugin free updates and technical support period has elapsed. Check our renewal plans and gain access to the growing list of exciting features, updates, bug fixes, security fixes and technical support.

							<br /><br />
							<a target="_blank" href="https://kainex.pl/renew-product-license/?checksum1=<?php echo WiseChatServiceConnector::getSettings(2) ?>&checksum2=<?php echo wp_hash(WISE_CHAT_PRO_NAME); ?>" class="button button-primary button-large" title="Check our plans">Click to learn more</a>
						</span>
					</p>
				</div>
			</div>
